...
namespace Mist{
  ...
  long unsigned int InOutBase::getMainSelectedTrack(){
    ...
    for (std::set<long unsigned int>::iterator it = selectedTracks.begin(); it != selectedTracks.end(); it++){
      if (myMeta.tracks.count(*it) && myMeta.tracks[*it].type == "video"){return *it;}
    }
  ...
  bool negotiationProxy::bufferStart(unsigned long tid, unsigned long pageNumber, DTSC::Meta &myMeta){
    ...
    // Nobody likes nasty data leaks.
    {
      ...
      if (!checkPage.mapped){
        WARN_MSG("Buffer deleted %s@%lu (%s) index. Re-negotiating...", streamName.c_str(), mapTid,
                 myMeta.tracks[tid].codec.c_str());
    ...
    if (!pagesByTrack[tid].count(pageNumber)){
      WARN_MSG("Aborting page buffer start: %lu is not a valid page number on track %lu~>%lu.",
               pageNumber, tid, mapTid);
      std::stringstream test;
      for (std::map<unsigned long, DTSCPageData>::iterator it = pagesByTrack[tid].begin();
           it != pagesByTrack[tid].end(); it++){
        test << it->first << " ";
      }
  ...
  void InOutBase::bufferRemove(unsigned long tid, unsigned long pageNumber){
    ...
    if (i == 1024){
      ERROR_MSG("Could not erase page %lu for track %lu->%lu stream %s from track index!",
                pageNumber, tid, mapTid, streamName.c_str());
    }
    ...
    if (!nProxy.pagesByTrack[tid].count(pageNumber)){
      INFO_MSG("Can't remove page %lu on track %lu~>%lu as it is not a valid page number.",
               pageNumber, tid, mapTid);
      return;
    }
  ...
  unsigned long negotiationProxy::bufferedOnPage(unsigned long tid, unsigned long keyNum){
    ...
    for (int i = 0; i < len; ++i){
      ...
      if (pageNum <= keyNum && keyNum < pageNum + keyAmount){return pageNum;}
    }
    return 0;
  }
  ...
  void InOutBase::bufferNext(const DTSC::Packet &pack){nProxy.bufferNext(pack, myMeta);}
  ...
  void negotiationProxy::bufferNext(const DTSC::Packet &pack, DTSC::Meta &myMeta){
    ...
    if (myMeta.live && pack.getTime() < myMeta.tracks[tid].lastms){
      DEBUG_MSG(multiWrong ? DLVL_HIGH : DLVL_WARN, "Wrong order on track %lu ignored: %lu < %lu",
                tid, pack.getTime(), myMeta.tracks[tid].lastms);
      multiWrong = true;
      return;
    }
    unsigned long mapTid = trackMap[tid];
    ...
    if (pageData.dataSize - curOffset < pack.getDataLen()){
      FAIL_MSG("Trying to buffer a packet (%llums) on page %lu for track %lu~>%lu, but we have a "
               "size mismatch. The packet is %d bytes long, so won't fit at offset %lu on a page "
               "of %llu bytes!",
               pack.getTime(), curPageNum[tid], tid, mapTid, pack.getDataLen(), curOffset, pageData.dataSize);
      return;
    }
  ...
  void InOutBase::bufferFinalize(unsigned long tid){nProxy.bufferFinalize(tid, myMeta);}
  ...
  void negotiationProxy::bufferFinalize(unsigned long tid, DTSC::Meta &myMeta){
    ...
    for (int i = 0; i < 1024; i++){
      ...
      if (!keyNum) continue;
      if (!lowest || keyNum < lowest){lowest = keyNum;}
    }
    ...
    if (!inserted){
      FAIL_MSG(
          "Can't register %lu on the metaPage of %s track %lu~>%lu, No empty spots left. Deleting.",
          curPageNum[tid], streamName.c_str(), tid, mapTid);
      // Since the buffer can't see it - we should delete it ourselves, now.
      curPage[tid].master = true;
    }else{
      HIGH_MSG("Registered %lu on the metaPage of %s track %lu~>%lu.", curPageNum[tid],
               streamName.c_str(), tid, mapTid);
#if defined(__CYGWIN__) || defined(_WIN32)
  ...
  void negotiationProxy::bufferLivePacket(const DTSC::Packet &packet, DTSC::Meta &myMeta){
    ...
    VERYHIGH_MSG("Buffering %s packet on track %lu: %llums, %db", myMeta.tracks[tid].codec.c_str(),
                 tid, packet.getTime(), packet.getPayloadLen());
  ...
  void negotiationProxy::bufferSinglePacket(const DTSC::Packet &packet, DTSC::Meta &myMeta){
    ...
    if (myMeta.tracks[tid].type == "video"){
      ...
      if (!pagesByTrack.count(tid) || pagesByTrack[tid].size() == 0){
        ...
        if (packet.getTime() - lastKey > AUDIO_KEY_INTERVAL){isKeyframe = true;}
      }
    }
    ...
    if (myMeta.live){
      if (packet.getTime() < myMeta.tracks[tid].lastms){
        HIGH_MSG("Wrong order on track %lu ignored: %lu < %lu", tid, packet.getTime(),
    ...
    if (isKeyframe && trackState[tid] == FILL_ACC){
      ...
      if ((tmpIt->second.curOffset > FLIP_DATA_PAGE_SIZE ||
           (packet.getTime() - tmpIt->second.firstTime) > FLIP_TARGET_DURATION) &&
          tmpIt->second.keyNum > 0){
  ...
  void InOutBase::continueNegotiate(){nProxy.continueNegotiate(myMeta);}
  ...
  void negotiationProxy::continueNegotiate(DTSC::Meta &myMeta){
    for (std::map<unsigned int, DTSC::Track>::iterator it = myMeta.tracks.begin();
      ...
      if (!trackState.count(it->first) ||
          (trackState[it->first] != FILL_ACC && trackState[it->first] != FILL_DEC)){
  ...
  void negotiationProxy::continueNegotiate(unsigned long tid, DTSC::Meta &myMeta, bool quickNegotiate){
    if (!tid){return;}
    if (userClient.getData()){userClient.keepAlive();}
    ...
    if (!trackOffset.count(tid)){
      ...
      for (int i = 0; i < SIMUL_TRACKS; i++){
        bool isFree = true;
        for (std::map<unsigned long, unsigned long>::iterator it = trackOffset.begin();
             it != trackOffset.end(); it++){
          if (it->second == i){
            isFree = false;
            break;
          }
    ...
    if (!tmp){
      DEBUG_MSG(DLVL_FAIL, "Failed to negotiate for incoming track %lu, there does not seem to be a connection with the buffer",
                tid);
      return;
    }
#if defined(__CYGWIN__) || defined(_WIN32)
    ...
    if (!trackState.count(tid)){
      ...
      if (quickNegotiate){
        ...
        MEDIUM_MSG("Buffer has indicated that incoming track %lu should start writing on track "
                   "%lu, page %lu",
                   tid, finalTid, firstPage);
    ...
    case FILL_NEW:{
      unsigned long newTid = ((long)(tmp[offset]) << 24) | ((long)(tmp[offset + 1]) << 16) |
                             ((long)(tmp[offset + 2]) << 8) | tmp[offset + 3];
      ...
      if (!myMeta.tracks[tid].type.size() || !myMeta.tracks[tid].codec.size()){
        FAIL_MSG("Negotiating a track without metadata. This is a serious issue, please report "
...
// Not actually removing the page, because we set master to false
    ...
    case FILL_NEG:{
      unsigned long finalTid = ((long)(tmp[offset]) << 24) | ((long)(tmp[offset + 1]) << 16) |
                               ((long)(tmp[offset + 2]) << 8) | tmp[offset + 3];
      ...
      finalTid = ((long)(tmp[offset]) << 24) | ((long)(tmp[offset + 1]) << 16) |
                 ((long)(tmp[offset + 2]) << 8) | tmp[offset + 3];
      ...
      firstPage++;
      MEDIUM_MSG("Buffer says %s:%lu should start writing on track %lu, page %lu",
                 streamName.c_str(), tid, finalTid, firstPage);
      trackMap[tid] = finalTid;
      if (myMeta.tracks.count(finalTid) && myMeta.tracks[finalTid].lastms){
        myMeta.tracks[finalTid].lastms = 0;
      }
      ...
      if (!metaPages[tid].mapped){metaPages[tid].init(pageName, SHM_TRACK_INDEX_SIZE, true);}
      metaPages[tid].master = false;
