  ...
  for i in ${start_streams[@]}; do
    ...
    if [[ "${INPUT:0:7}" == "file://" ]]; then
      ...
      if [[ "$PRESET" == "raw" ]]; then
        ...
        tmpcommand="${tmpcommand} fmpeg -i "$INPUT" -re -acodec aac -ar 11025 -vcodec libx264 -b 1500k -vpre ultrafast -refs 1 -bf 0 -g 150 -f flv - |" ffmpeg -re 
      else
        tmpcommand="${tmpcommand} fmpeg -i "$INPUT" -re -acodec aac -ar 11025 -vcodec libx264 -b 700k -vpre ultrafast -refs 1 -bf 0 -g 150 -f flv - |" ffmpeg -re 
      fi
