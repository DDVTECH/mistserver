      ...
      container.updateBuffers = function(buffers){
        ...
        if (buffers) {
          ...
          for (var i = 0; i < buffers.length; i++) {
            ...
            //out.push([MistUtil.format.time(buffers.start(i),{ms:true}),(buffers.end(i)-buffers.start(i))*1e3+"ms"]);
          }

          //console.warn("Buffers",out);
        }
      };
        ...
        var videovalues = {
          ...
          "Tracks playing": function(){
            if (MistVideo.reporting) {
              if (MistVideo.reporting.stats && MistVideo.reporting.stats.d && MistVideo.reporting.stats.d.tracks) {
                ...
                if (MistVideo.info && MistVideo.info.meta && MistVideo.info.meta.tracks) { 
                  ...
                  for (var i in meta) {
                    var t = meta[i];
                    if (trackidxs.indexOf(t.idx) > -1) {
                      out[t.type] = t;
                    }
                  }
                  return processMultiOutput(out,function(t){
                    return t.codec+" ("+t.idx+")";
                  });
                }
                return MistVideo.reporting.stats.d.tracks.join(", ");
              }
              return "";
            }
          },
            ...
            if (MistVideo.player.api && ("buffer_manager" in MistVideo.player.api)) {
              ...
              if (bm) {
                ...
                return {
                  ...
                  val: Math.round(cb)+"/"+Math.round(bm.desiredBuffer)+"ms"
                };
                return Math.round(bm.buffer())+"/"+Math.round(bm.desiredBuffer)+"ms";
              }
              return 0;
            }
          },
