...
namespace Mist{
  ...
  uint64_t OutProgressiveMP4::estimateFileSize(){
    ...
    for (std::set<unsigned long>::iterator it = selectedTracks.begin(); it != selectedTracks.end(); it++){
      for (std::deque<unsigned long>::iterator keyIt = myMeta.tracks[*it].keySizes.begin();
           keyIt != myMeta.tracks[*it].keySizes.end(); keyIt++){
        retVal += *keyIt;
      }
    }
    return retVal * 1.1;
  ...
  uint64_t OutProgressiveMP4::mp4HeaderSize(uint64_t &fileSize, int fragmented){
    ...
    for (std::set<unsigned long>::iterator it = selectedTracks.begin(); it != selectedTracks.end(); it++){
      if (myMeta.tracks[*it].firstms < firstms){firstms = myMeta.tracks[*it].firstms;}
    }
    for (std::set<unsigned long>::iterator it = selectedTracks.begin(); it != selectedTracks.end(); it++){
  ...
  std::string OutProgressiveMP4::DTSCMeta2MP4Header(uint64_t &size, int fragmented){
    ...
    for (std::set<unsigned long>::iterator it = selectedTracks.begin(); it != selectedTracks.end(); it++){
      ...
      if (fragmented){tkhdBox.setDuration(-1);}
      ...
      if (!fragmented){
        ...
        for (size_t part = 0; part < partCount; ++part){
          ...
          if (thisTrack.type == "meta"){partSize += 2;}
    ...
    if (fragmented){
      ...
      for (std::deque<MP4::TRAK>::iterator trakIt = trak.begin(); trakIt != trak.end(); trakIt++){
        ...
        if (useLargeBoxes){
          checkCO64Boxes.insert(
              std::pair<size_t, MP4::CO64>(tkhdBox.getTrackID(), stblBox.getChild<MP4::CO64>()));
        }else{
          checkStcoBoxes.insert(
              std::pair<size_t, MP4::STCO>(tkhdBox.getTrackID(), stblBox.getChild<MP4::STCO>()));
        }
      }
      ...
      while (!sortSet.empty()){
        ...
        if (thisTrack.type == "meta"){dataSize += 2;}
  ...
  void OutProgressiveMP4::findSeekPoint(uint64_t byteStart, uint64_t &seekPoint, uint64_t headerSize){
    ...
    while (!sortSet.empty()){
      ...
      if (myMeta.tracks[temp.trackID].codec == "subtitle"){partSize += 2;}
  ...
  void OutProgressiveMP4::sendFragmentHeader(){
    ...
    for (std::map<size_t, fragSet>::iterator it = currentPartSet.begin(); it != currentPartSet.end(); it++){
      if (myMeta.tracks[it->first].type == "video"){
        ...
        if (myMeta.tracks[it->first].type == "audio"){hasAudio = true;}
        sortedTracks.push_back(it);
      }
    }

    for (std::deque<std::map<size_t, fragSet>::iterator>::iterator ti = sortedTracks.begin();
         ti != sortedTracks.end(); ++ti){
      ...
      for (std::set<keyPart>::iterator trunIt = sortSet.begin(); trunIt != sortSet.end(); trunIt++){
        if (trunIt->trackID == tid){
          ...
          if (keysOnly){partDur = keysOnly;}

          MP4::TRUN trunBox;
          trunBox.setFlags(MP4::trundataOffset | MP4::trunfirstSampleFlags | MP4::trunsampleSize |
                           MP4::trunsampleDuration | (partOffset ? MP4::trunsampleOffsets : 0));
          ...
          if (partOffset){sampleInfo.sampleOffset = partOffset;}
  ...
  /// using the fragment number **FOR THIS USER, NOT ACTUAL FRAGMENT NUMBER, HAS NOTHING TO DO WITH
  /// ACTUAL FRAGMENTS EVEN** We take the corresponding keyframe and interframes of the main video
  /// track and take concurrent frames from its secondary (audio) tracks \todo See if we can use
  ///something more elegant than a member variable...
  ...
  void OutProgressiveMP4::sendNext(){
    ...
    if ((unsigned long)thisPacket.getTrackId() != thisPart.trackID ||
        thisPacket.getTime() != thisPart.time || len != thisPart.size){
      if (thisPacket.getTime() > sortSet.begin()->time || thisPacket.getTrackId() > sortSet.begin()->trackID){
        if (perfect){
          WARN_MSG("Warning: input is inconsistent. Expected %lu:%lu but got %ld:%llu - cancelling "
                   "playback",
                   thisPart.trackID, thisPart.time, thisPacket.getTrackId(), thisPacket.getTime());
          perfect = false;
          myConn.close();
        }
      }else{
        WARN_MSG(
            "Did not receive expected %lu:%lu (%lub) but got %ld:%llu (%ub) - throwing it away",
            thisPart.trackID, thisPart.time, thisPart.size, thisPacket.getTrackId(),
            thisPacket.getTime(), len);
      }
      return;
    }
    ...
    if (currPos >= byteStart){
      ...
      if (currPos + (long long)len > byteStart){
        myConn.SendNow(dataPointer + (byteStart - currPos),
                       std::min(leftOver, (int64_t)(len - (byteStart - currPos))));
