...
namespace Loadbalancer{
  ...
  void configFromString(std::string s){
    ...
    if (j.isMember(configFallback)) fallback = j[configFallback].asString();
    if (j.isMember(configC)) weight_cpu = j[configC].asInt();
    if (j.isMember(configR)) weight_ram = j[configR].asInt();
    if (j.isMember(configBW)) weight_bw = j[configBW].asInt();
    if (j.isMember(configWG)) weight_geo = j[configWG].asInt();
    if (j.isMember(configWB)) weight_bonus = j[configWB].asInt();
    if (j.isMember(configPass)) passHash = j[configPass].asString();
    if (j.isMember(configSPass)) passphrase = j[configSPass].asStringRef();
    if (j.isMember(configBearer)) bearerTokens = j[configBearer].asStringSet();

    // balancing
    if (j.isMember(configMinStandby)) minstandby = j[configMinStandby].asInt();
    if (j.isMember(configMaxStandby)) maxstandby = j[configMaxStandby].asInt();
    ...
    if (j.isMember(configBalancingInterval)) balancingInterval = j[configBalancingInterval].asInt();
    if (j.isMember(serverMonitorLimitKey)) serverMonitorLimit = j[serverMonitorLimitKey].asInt();
    ...
    // load whitelist
    if (j.isMember(configWhitelist)) whitelist = j[configWhitelist].asStringSet();
    if (j.isMember(configUsers)) userAuth = j[configUsers].asStringPairMap();
