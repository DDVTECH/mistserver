...
namespace Controller{
  ...
  /// Gets the last error-level or lower log message generated by any of the given PIDs
  std::string findLastErrorForPids(std::set<uint64_t> & pids){
    for (uint64_t i = logCounter; i > rlxLogs->getDeleted(); --i) {
      const char *msg = rlxLogs->getPointer(logFMsg, i);
      if (!msg[0]) { continue; }
      const uint64_t pid = rlxLogs->getInt(logFPID, i);
      if (!pids.count(pid)) { continue; }
      const char *kind = rlxLogs->getPointer(logFKind, i);
      if (!strcmp(kind, "FAIL") || !strcmp(kind, "ERROR")) { return msg; }
    }
    return "";
  }
