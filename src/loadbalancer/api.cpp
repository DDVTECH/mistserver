...
void updateHost(JSON::Value newVals){
  ...
  if (i == hosts.end()){
    ...
    (*i)->details->update(
        newVals[fillstateout], newVals[fillStreamOut], newVals[scoreSourceKey].asInt(),
        newVals[scoreRateKey].asInt(), newVals[outputsKey], newVals[confStreamKey],
        newVals[streamsKey], newVals[tagKey], newVals[cpuKey].asInt(), newVals[servLatiKey].asDouble(),
        newVals[servLongiKey].asDouble(), newVals[binhostKey].asString().c_str(),
        newVals[hostKey].asString(), newVals[toAddKey].asInt(), newVals[currBandwidthKey].asInt(),
        newVals[availBandwidthKey].asInt(), newVals[currRAMKey].asInt(), newVals[ramMaxKey].asInt());
  }
}
