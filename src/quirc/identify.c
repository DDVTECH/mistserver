...
static void perspective_unmap(const quirc_float_t *c, const struct quirc_point *in, quirc_float_t *u, quirc_float_t *v) {
  ...
  quirc_float_t den =
    (quirc_float_t)1 / (-c[0] * c[7] * y + c[1] * c[6] * y + (c[3] * c[7] - c[4] * c[6]) * x + c[0] * c[4] - c[1] * c[3]);
...
static struct quirc_flood_fill_vars *flood_fill_call_next(struct quirc *q, quirc_pixel_t *row, int from, int to, span_func_t func,
                                                          void *user_data, struct quirc_flood_fill_vars *vars, int direction) {
  int *leftp;
  ...
  while (*leftp <= vars->right) {
    if (row[*leftp] == from) {
      ...
      /* Fill the extent */
      flood_fill_line(q, *leftp, next_vars->y, from, to, func, user_data, &next_left, &next_vars->right);
...
static uint8_t otsu(const struct quirc *q) {
  ...
  unsigned int i = 0;
  for (i = 0; i <= UINT8_MAX; ++i) { sum += i * histogram[i]; }
...
static void find_region_corners(struct quirc *q, int rcode, const struct quirc_point *ref, struct quirc_point *corners) {
  ...
  for (i = 0; i < 4; i++) memcpy(&psd.corners[i], &region->seed, sizeof(psd.corners[i]));
...
static int fitness_all(const struct quirc *q, int index) {
  ...
  for (i = 1; i < ap_count; i++)
    for (j = 1; j < ap_count; j++) score += fitness_apat(q, index, info->apat[i], info->apat[j]);
...
static void jiggle_perspective(struct quirc *q, int index) {
  ...
  for (i = 0; i < 8; i++) adjustments[i] = qr->c[i] * (quirc_float_t)0.02;
...
static void rotate_capstone(struct quirc_capstone *cap, const struct quirc_point *h0, const struct quirc_point *hd) {
  ...
  /* Rotate the capstone */
  for (j = 0; j < 4; j++) memcpy(&copy[j], &cap->corners[(j + best) % 4], sizeof(copy[j]));
...
static void test_neighbours(struct quirc *q, int i, const struct neighbour_list *hlist, const struct neighbour_list *vlist) {
  ...
  for (int j = 0; j < hlist->count; j++) {
    ...
    for (int k = 0; k < vlist->count; k++) {
      ...
      if (squareness < (quirc_float_t)0.2) record_qr_grid(q, hn->index, i, vn->index);
    }
  }
}
...
void quirc_extract(const struct quirc *q, int index, struct quirc_code *code) {
   ...
   */
  if (code->size > QUIRC_MAX_GRID_SIZE) return;

  for (y = 0; y < qr->grid_size; y++) {
    int x;
    for (x = 0; x < qr->grid_size; x++) {
      if (read_cell(q, index, x, y) > 0) { code->cell_bitmap[i >> 3] |= (1 << (i & 7)); }
      i++;
    }
  }
}
