...
static quirc_decode_error_t read_format(const struct quirc_code *code, struct quirc_data *data, int which) {
  ...
  if (which) {
    for (i = 0; i < 7; i++) format = (format << 1) | grid_bit(code, 8, code->size - 1 - i);
    for (i = 0; i < 8; i++) format = (format << 1) | grid_bit(code, code->size - 8 + i, 8);
  } else {
    static const int xs[15] = {8, 8, 8, 8, 8, 8, 8, 8, 7, 5, 4, 3, 2, 1, 0};
    static const int ys[15] = {0, 1, 2, 3, 4, 5, 7, 8, 8, 8, 8, 8, 8, 8, 8};

    for (i = 14; i >= 0; i--) format = (format << 1) | grid_bit(code, xs[i], ys[i]);
  }
...
static void read_data(const struct quirc_code *code, struct quirc_data *data, struct datastream *ds) {
  ...
  while (x > 0) {
    ...
    if (!reserved_cell(data->version, y, x - 1)) read_bit(code, data, ds, y, x - 1);
...
static quirc_decode_error_t decode_numeric(struct quirc_data *data, struct datastream *ds) {
  ...
  if (data->payload_len + count + 1 > QUIRC_MAX_PAYLOAD) return QUIRC_ERROR_DATA_OVERFLOW;
...
static quirc_decode_error_t decode_byte(struct quirc_data *data, struct datastream *ds) {
  ...
  for (i = 0; i < count; i++) data->payload[data->payload_len++] = take_bits(ds, 8);
...
static quirc_decode_error_t decode_kanji(struct quirc_data *data, struct datastream *ds) {
  ...
  count = take_bits(ds, bits);
  if (data->payload_len + count * 2 + 1 > QUIRC_MAX_PAYLOAD) return QUIRC_ERROR_DATA_OVERFLOW;
...
quirc_decode_error_t quirc_decode(const struct quirc_code *code, struct quirc_data *data) {
  ...
  if (code->size > QUIRC_MAX_GRID_SIZE) return QUIRC_ERROR_INVALID_GRID_SIZE;
  ...
  if (data->version < 1 || data->version > QUIRC_MAX_VERSION) return QUIRC_ERROR_INVALID_VERSION;
...
void quirc_flip(struct quirc_code *code) {
  ...
  for (int y = 0; y < code->size; y++) {
    for (int x = 0; x < code->size; x++) {
      if (grid_bit(code, y, x)) { flipped.cell_bitmap[offset >> 3u] |= (1u << (offset & 7u)); }
      offset++;
    }
  }
