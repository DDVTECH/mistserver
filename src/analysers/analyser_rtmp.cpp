...
bool AnalyserRTMP::parsePacket(){
  ...
  while(true){
    ...
    case 0: // does not exist
      DETAIL_LOW("Error chunk @ %" PRIu64 " - CS%i, T%" PRIu64 ", L%i, LL%i, MID%i", mediaDown - strbuf.size(),
                 next.cs_id, next.timestamp, next.real_len, next.len_left, next.msg_stream_id);
      return 0;
      break; // happens when connection breaks unexpectedly
    case 1:  // set chunk size
      RTMPStream::chunk_rec_max = ntohl(*(int *)next.data.c_str());
      DETAIL_MED("CTRL: Set chunk size: %lu", RTMPStream::chunk_rec_max);
      break;
    ...
    case 3: // ack
      ...
      DETAIL_MED("CTRL: Acknowledgement: %lu", RTMPStream::snd_window_at);
      break;
    case 4:{
    ...
    case 5: // window size of other end
      ...
      DETAIL_MED("CTRL: Window size: %lu", RTMPStream::rec_window_size);
      break;
    case 6:
      RTMPStream::snd_window_size = ntohl(*(int *)next.data.c_str());
      // 4 bytes window size, 1 byte limit type (ignored)
      DETAIL_MED("CTRL: Set peer bandwidth: %lu", RTMPStream::snd_window_size);
      break;
    case 8:
    case 9:
      if (detail >= 4 || reconstruct.good() || validate){
        F.ChunkLoader(next);
        if (F.tagTime() > mediaTime){mediaTime = F.tagTime();}
