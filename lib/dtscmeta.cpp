...
namespace DTSC{
  ...
  void Packet::genericFill(long long packTime, long long packOffset, long long packTrack,
                           const char *packData, long long packDataSize, uint64_t packBytePos,
                           bool isKeyframe, int64_t bootMsOffset){
    null();
    master = true;
    ...
    if (packData && packDataSize < 1){
      FAIL_MSG("Attempted to fill a packet with %lli bytes for timestamp %llu, track %llu!",
               packDataSize, packTime, packTrack);
      return;
    }
    unsigned int sendLen = 24 + (packOffset ? 17 : 0) + (packBytePos ? 15 : 0) +
                           (isKeyframe ? 19 : 0) + (bootMsOffset ? 14 : 0) + packDataSize + 11;
    resize(sendLen);
  ...
  size_t Packet::getDataLen() const{return dataLen;}
  ...
  void Scan::nullMember(const std::string &indice){nullMember(indice.data(), indice.size());}
  ...
  Ivec::Ivec(long long int iVec){setIvec(iVec);}

  void Ivec::setIvec(long long int iVec){Bit::htobll(data, iVec);}
  ...
  long long int Ivec::asInt(){return Bit::btohll(data);}

  char *Ivec::getData(){return data;}
  /*LTS-END*/

  ///\brief Returns the payloadsize of a part
  uint32_t Part::getSize(){return Bit::btoh24(data);}

  ///\brief Sets the payloadsize of a part
  void Part::setSize(uint32_t newSize){Bit::htob24(data, newSize);}

  ///\brief Returns the duration of a part
  uint32_t Part::getDuration(){return Bit::btoh24(data + 3);}

  ///\brief Sets the duration of a part
  void Part::setDuration(uint32_t newDuration){Bit::htob24(data + 3, newDuration);}
  ...
  void Part::setOffset(uint32_t newOffset){Bit::htob24(data + 6, newOffset);}

  ///\brief Returns the data of a part
  char *Part::getData(){return data;}
  ...
  void Part::toPrettyString(std::ostream &str, int indent){
    str << std::string(indent, ' ') << "Part: Size(" << getSize() << "), Dur(" << getDuration()
        << "), Offset(" << getOffset() << ")" << std::endl;
  }

  ///\brief Returns the byteposition of a keyframe
  unsigned long long Key::getBpos(){return Bit::btohll(data);}

  void Key::setBpos(unsigned long long newBpos){Bit::htobll(data, newBpos);}

  unsigned long Key::getLength(){return Bit::btoh24(data + 8);}

  void Key::setLength(unsigned long newLength){Bit::htob24(data + 8, newLength);}

  ///\brief Returns the number of a keyframe
  unsigned long Key::getNumber(){return Bit::btohl(data + 11);}

  ///\brief Sets the number of a keyframe
  void Key::setNumber(unsigned long newNumber){Bit::htobl(data + 11, newNumber);}

  ///\brief Returns the number of parts of a keyframe
  unsigned short Key::getParts(){return Bit::btohs(data + 15);}

  ///\brief Sets the number of parts of a keyframe
  void Key::setParts(unsigned short newParts){Bit::htobs(data + 15, newParts);}

  ///\brief Returns the timestamp of a keyframe
  unsigned long long Key::getTime(){return Bit::btohll(data + 17);}

  ///\brief Sets the timestamp of a keyframe
  void Key::setTime(unsigned long long newTime){Bit::htobll(data + 17, newTime);}

  ///\brief Returns the data of this keyframe struct
  char *Key::getData(){return data;}

  ///\brief Converts a keyframe to a human readable string
  ///\param str The stringstream to append to
  ///\param indent the amount of indentation needed
  void Key::toPrettyString(std::ostream &str, int indent){
    str << std::string(indent, ' ') << "Key " << getNumber() << ": Pos(" << getBpos() << "), Dur("
        << getLength() << "), Parts(" << getParts() << "), Time(" << getTime() << ")" << std::endl;
  }

  ///\brief Returns the duration of this fragment
  unsigned long Fragment::getDuration(){return Bit::btohl(data);}

  ///\brief Sets the duration of this fragment
  void Fragment::setDuration(unsigned long newDuration){Bit::htobl(data, newDuration);}

  ///\brief Returns the length of this fragment
  char Fragment::getLength(){return data[4];}

  ///\brief Sets the length of this fragment
  void Fragment::setLength(char newLength){data[4] = newLength;}

  ///\brief Returns the number of the first keyframe in this fragment
  unsigned long Fragment::getNumber(){return Bit::btohl(data + 5);}

  ///\brief Sets the number of the first keyframe in this fragment
  void Fragment::setNumber(unsigned long newNumber){Bit::htobl(data + 5, newNumber);}

  ///\brief Returns the size of a fragment
  unsigned long Fragment::getSize(){return Bit::btohl(data + 9);}

  ///\brief Sets the size of a fragement
  void Fragment::setSize(unsigned long newSize){Bit::htobl(data + 9, newSize);}

  ///\brief Returns thte data of this fragment structure
  char *Fragment::getData(){return data;}

  ///\brief Converts a fragment to a human readable string
  ///\param str The stringstream to append to
  ///\param indent the amount of indentation needed
  void Fragment::toPrettyString(std::ostream &str, int indent){
    str << std::string(indent, ' ') << "Fragment " << getNumber() << ": Dur(" << getDuration()
        << "), Len(" << (int)getLength() << "), Size(" << getSize() << ")" << std::endl;
  }
  ...
  Track::Track(JSON::Value &trackRef){
    ...
    if (trackRef.isMember("keysizes") && trackRef["keysizes"].isString()){
      ...
      for (unsigned int i = 0; i < tmp.size(); i += 4){
        keySizes.push_back((((long unsigned)tmp[i]) << 24) | (((long unsigned)tmp[i + 1]) << 16) |
                           (((long unsigned int)tmp[i + 2]) << 8) | tmp[i + 3]);
      }
    }
  ...
  Track::Track(Scan &trackRef){
    ...
    if (trackRef.getMember("lang")){lang = trackRef.getMember("lang").asString();}
    if (type == "audio"){
  ...
  /*LTS
  void Track::update(long long packTime, long long packOffset, long long packDataSize, uint64_t
  packBytePos, bool isKeyframe, long long packSendSize, unsigned long segment_size){LTS*/
  void Track::update(long long packTime, long long packOffset, long long packDataSize, uint64_t packBytePos,
                     bool isKeyframe, long long packSendSize, unsigned long segment_size, const char *iVec){
    if ((unsigned long long)packTime < lastms){
      static bool warned = false;
      if (!warned){
        ERROR_MSG("Received packets for track %u in wrong order (%lld < %llu) - ignoring! Further "
                  "messages on HIGH level.",
                  trackID, packTime, lastms);
        warned = true;
      }else{
        HIGH_MSG("Received packets for track %u in wrong order (%lld < %llu) - ignoring! Further "
                 "messages on HIGH level.",
                 trackID, packTime, lastms);
      }
      return;
    ...
    lastms = packTime;
    if (isKeyframe || !keys.size() ||
        (type != "video" && packTime >= AUDIO_KEY_INTERVAL &&
         packTime - (unsigned long long)keys[keys.size() - 1].getTime() >= AUDIO_KEY_INTERVAL)){
      Key newKey;
      ...
      if (!fragments.size() || ((unsigned long long)packTime > segment_size &&
                                (unsigned long long)packTime - segment_size >=
                                    (unsigned long long)getKey(fragments.rbegin()->getNumber()).getTime())){
        // new fragment
        Fragment newFrag;
        ...
        if (fragments.size()){
          fragments[fragments.size() - 1].setDuration(
              packTime - getKey(fragments[fragments.size() - 1].getNumber()).getTime());
          ...
          max_bps = std::max(max_bps, (int)((fragments.rbegin()->getSize() * 1000) /
                                            fragments.rbegin()->getDuration()));
        }
        newFrag.setDuration(0);
        newFrag.setSize(0);
  ...
  void Track::removeFirstKey(){
    ...
    for (int i = 0; i < keys[0].getParts(); i++){parts.pop_front();}
  ...
  uint32_t Track::biggestFragment(){
    ...
    for (unsigned int i = 0; i < fragments.size(); i++){
      if (fragments[i].getDuration() > ret){ret = fragments[i].getDuration();}
    }
    return ret;
  }
  ...
  Key &Track::getKey(unsigned int keyNum){
    static Key empty;
    if (!keys.size() || keyNum < keys[0].getNumber()){return empty;}
    if ((keyNum - keys[0].getNumber()) > keys.size()){return empty;}
  ...
  uint32_t Track::timeToFragnum(uint64_t timestamp){
    uint32_t i = 0;
    for (std::deque<Fragment>::iterator it = fragments.begin(); it != fragments.end(); ++it){
      if (timestamp < getKey(it->getNumber()).getTime() + it->getDuration()){return i;}
      ++i;
    }
  ...
  Meta::Meta(const DTSC::Packet &source){reinit(source);}
  ...
  void Meta::reinit(const DTSC::Packet &source){
    ...
    do{
      ...
      if (tmpTrack.asBool()){
        ...
        if (trackId){tracks[trackId] = Track(tmpTrack);}
        num++;
      }
  ...
  Meta::Meta(JSON::Value &meta){
    ...
    bufferWindow = 0;
    if (meta.isMember("buffer_window")){bufferWindow = meta["buffer_window"].asInt();}
    // for (JSON::ObjIter it = meta["tracks"].ObjBegin(); it != meta["tracks"].ObjEnd(); it++){
    jsonForEach(meta["tracks"], it){
      if ((*it)["trackid"].asInt()){tracks[(*it)["trackid"].asInt()] = Track((*it));}
    }
  ...
  void Meta::update(JSON::Value &pack, unsigned long segment_size){
    /*LTS
    update(pack["time"].asInt(), pack.isMember("offset")?pack["offset"].asInt():0,
    pack["trackid"].asInt(), pack["data"].asStringRef().size(),
    pack.isMember("bpos")?pack["bpos"].asInt():0, pack.isMember("keyframe"), pack.packedSize(),
    segment_size); LTS*/
    update(pack["time"].asInt(), pack.isMember("offset") ? pack["offset"].asInt() : 0,
           pack["trackid"].asInt(), pack["data"].asStringRef().size(),
           pack.isMember("bpos") ? pack["bpos"].asInt() : 0, pack.isMember("keyframe"), pack.packedSize(),
           segment_size, pack.isMember("ivec") ? pack["ivec"].asStringRef().data() : 0);
  }
  ...
  void Meta::update(const DTSC::Packet &pack, unsigned long segment_size){
    ...
    /*LTS
    update(pack.getTime(), pack.hasMember("offset")?pack.getInt("offset"):0, pack.getTrackId(),
    dataLen, pack.hasMember("bpos")?pack.getInt("bpos"):0, pack.hasMember("keyframe"),
    pack.getDataLen(), segment_size); LTS*/
    update(pack.getTime(), pack.hasMember("offset") ? pack.getInt("offset") : 0, pack.getTrackId(),
           dataLen, pack.hasMember("bpos") ? pack.getInt("bpos") : 0, pack.hasMember("keyframe"),
           pack.getDataLen(), segment_size, ivecLen ? ivec : 0);
    if (!bootMsOffset && pack.hasMember("bmo")){bootMsOffset = pack.getInt("bmo");}
  }
  ...
  void Meta::updatePosOverride(DTSC::Packet &pack, uint64_t bpos){
    ...
    /*LTS
    update(pack.getTime(), pack.hasMember("offset")?pack.getInt("offset"):0, pack.getTrackId(),
    dataLen, bpos, pack.hasMember("keyframe"), pack.getDataLen()); LTS*/
    char *ivec;
    size_t ivecLen;
    pack.getString("ivec", ivec, ivecLen);
    update(pack.getTime(), pack.hasMember("offset") ? pack.getInt("offset") : 0, pack.getTrackId(),
           dataLen, bpos, pack.hasMember("keyframe"), pack.getDataLen(), 1900, ivecLen ? ivec : 0);
  }

  void Meta::update(long long packTime, long long packOffset, long long packTrack,
                    long long packDataSize, uint64_t packBytePos, bool isKeyframe,
                    long long packSendSize, unsigned long segment_size, const char *ivec){
    if (nextIsKey){
      isKeyframe = true;
      nextIsKey = false;
    }
    DONTEVEN_MSG("Updating meta with: t=%lld, o=%lld, s=%lld, t=%lld, p=%lld", packTime, packOffset,
                 packDataSize, packTrack, packBytePos);
    if (!packSendSize){
      // time and trackID are part of the 20-byte header.
      // the container object adds 4 bytes (plus 2+namelen for each content, see below)
      // offset, if non-zero, adds 9 bytes (integer type) and 8 bytes (2+namelen)
      ...
      // keyframe, if true, adds 9 bytes (integer type) and 10 bytes (2+namelen)
      // data adds packDataSize+5 bytes (string type) and 6 bytes (2+namelen)
      packSendSize = 24 + (packOffset ? 17 : 0) + (packBytePos ? 15 : 0) + (isKeyframe ? 19 : 0) +
                     packDataSize + 11;
    }
    if (vod != (packBytePos > 0)){
      INFO_MSG("Changing stream from %s to %s (bPos=%lld)", vod ? "VoD" : "live",
               packBytePos ? "Vod" : "live", packBytePos);
    }
    vod = (packBytePos > 0);
    live = !vod;
    EXTREME_MSG("Updating meta with %lld@%lld+%lld", packTrack, packTime, packOffset);
    if (packTrack > 0 && tracks.count(packTrack)){
      tracks[packTrack].update(packTime, packOffset, packDataSize, packBytePos, isKeyframe,
                               packSendSize, segment_size, ivec);
    }
  }
  ...
  Track &Meta::mainTrack(){
    ...
    for (std::map<unsigned int, Track>::iterator it = tracks.begin(); it != tracks.end(); it++){
      if (it->second.type == "video"){return it->second;}
    }
    return tracks.begin()->second;
  }
  ...
  void Track::toPrettyString(std::ostream &str, int indent, int verbosity){
    ...
    if (lang.size()){str << std::string(indent + 2, ' ') << "Language: " << lang << std::endl;}
    if (type == "audio"){
    ...
    if (verbosity & 0x02){
      for (unsigned int i = 0; i < keys.size(); i++){keys[i].toPrettyString(str, indent + 4);}
    }
    ...
    if (verbosity & 0x04){
      for (unsigned int i = 0; i < parts.size(); i++){parts[i].toPrettyString(str, indent + 4);}
    }
  }
  ...
  std::string Track::getIdentifier(){
    ...
    if (lang.size() && lang != "und"){result << "_" << lang;}
    return result.str();
  }
  ...
  int Track::getSendLen(bool skipDynamic){
    ...
    if (lang.size() && lang != "und"){result += 11 + lang.size();}
    if (type == "audio"){
      result += 49;
    }else if (type == "video"){
      result += 48;
    }
    if (!skipDynamic && missedFrags){result += 23;}
    if (minKeepAway){result += 19;}
    return result;
  }
  ...
  void Track::writeTo(char *&p){
    ...
    if (fragments.size() && (&firstFrag) == 0){return;}
  ...
  unsigned int Meta::getSendLen(bool skipDynamic, std::set<unsigned long> selectedTracks){
    unsigned int dataLen =
        16 + (vod ? 14 : 0) + (live ? 15 : 0) + (merged ? 17 : 0) + (bufferWindow ? 24 : 0) + 21;
    for (std::map<unsigned int, Track>::iterator it = tracks.begin(); it != tracks.end(); it++){
  ...
  JSON::Value Track::toJSON(bool skipDynamic){
    ...
    result["init"] = init;
    if (lang.size() && lang != "und"){result["lang"] = lang;}
    result["trackid"] = trackID;
    result["firstms"] = firstms;
    result["lastms"] = lastms;
    result["bps"] = bps;
    result["maxbps"] = max_bps;
    if (missedFrags){result["missed_frags"] = missedFrags;}
    result["codec"] = codec;
    result["type"] = type;
    if (type == "audio"){
      result["rate"] = rate;
    ...
    if (minKeepAway){result["keepaway"] = minKeepAway;}
  ...
  JSON::Value Meta::toJSON(){
    ...
    if (vod){result["vod"] = 1;}
    if (live){result["live"] = 1;}
    if (merged){result["merged"] = 1;}
    if (bufferWindow){result["buffer_window"] = bufferWindow;}
    if (version){result["version"] = version;}
    if (bootMsOffset){result["bootoffset"] = bootMsOffset;}
    if (sourceURI.size()){result["source"] = sourceURI;}
    result["moreheader"] = moreheader;
    if (inputLocalVars){result["inputlocalvars"] = inputLocalVars;}
    return result;
  }
  ...
  void Meta::toPrettyString(std::ostream &str, int indent, int verbosity){
    ...
    if (vod){str << std::string(indent, ' ') << "Video on Demand" << std::endl;}
    if (live){str << std::string(indent, ' ') << "Live" << std::endl;}
    if (merged){str << std::string(indent, ' ') << "Merged file" << std::endl;}
    if (bufferWindow){
  ...
  PartIter::PartIter(Track &Trk, Fragment &frag){
    ...
    while (kIt->getNumber() < fragNum && kIt != tRef->keys.end()){
      ...
      for (uint32_t pCount = 0; pCount < kParts && pIt != tRef->parts.end(); ++pCount){++pIt;}
      ++kIt;
    }
  ...
  Part &PartIter::operator*() const{
    if (tRef && pIt != tRef->parts.end()){return *pIt;}
    static Part error;
  ...
  /// Dereferences into a Value reference.
  /// If invalid iterator, returns an empty reference and prints a warning message.
  Part *PartIter::operator->() const{return &(operator*());}

  /// True if not done iterating.
  PartIter::operator bool() const{return (tRef && pIt != tRef->parts.end());}
  ...
  PartIter &PartIter::operator++(){
    if (*this){
      ...
      if (++currInKey >= kIt->getParts()){
        ...
        if (++kIt == tRef->keys.end() || kIt->getNumber() >= lastKey){tRef = 0;}
      }
    }
    return *this;
  }
